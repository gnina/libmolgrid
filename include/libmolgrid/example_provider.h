 ...
 * consists of some number of numerical labels (num_labels, will be
 * auto-detected if not specified) followed by file paths to molecular data, all
 * space separated.
 */
...
public:
  ...
  static std::shared_ptr<ExampleRefProvider>
  createProvider(const ExampleProviderSettings &settings);
  ...
  ExampleProvider(std::shared_ptr<ExampleRefProvider> p,
  ...
  virtual void populate(const std::vector<std::string> &fnames,
                        int num_labels = -1);

  /// return number of labels for each example (computed from first example
  /// only)
  virtual size_t num_labels() const { return provider->num_labels(); }

  /// provide next example
  virtual void next(Example &ex);
  virtual Example next() {
    Example ex;
    next(ex);
    return ex;
  }
   ...
   *  Note this is the epoch of the next example to be provided, not the
   * previous.
   */
  virtual size_t get_small_epoch_num() const {
    return provider->get_small_epoch_num();
  }
  ...
  /// provide a batch of examples, unspecified or 0 batch_size uses default
  /// batch size
  ...
  std::vector<std::string> get_type_names() const {
    return extractor.get_type_names();
  }
  ...
  void save_mem_caches(const std::string &fname) const {
    extractor.save_mem_caches(fname);
  }

  /** \brief Read contents of input stream into memory caches.
   * @param[in] in input stream
   */
  void load_mem_caches(std::istream &in) { extractor.load_mem_caches(in); }
  /** \brief Read contents of provided file into memory caches.
   * @param[in] fname file name
   */
  void load_mem_caches(const std::string &fname) { extractor.load_mem_caches(fname); }

  size_t mem_caches_size() const { return extractor.mem_caches_size(); }
};
