...
void ExampleExtractor::extract(const ExampleRef &ref, Example &ex) {
  ...
  if (!duplicate_poses || ref.files.size() < 3) {
    ...
    for (unsigned i = 0, n = ref.files.size(); i < n; i++) {
      ...
      unsigned t = i;
      if (t >= coord_caches.size())
        t = coord_caches.size() - 1; // repeat last typer if necessary
      coord_caches[t].set_coords(fname, ex.sets[i]);
    }
  } else { // duplicate first pose (receptor) to match each of the remaining
           // poses
...
void ExampleExtractor::save_mem_caches(std::ostream &out) const {
  for (const auto &c : coord_caches) {
    c.save_mem_cache(out);
  }
}

void ExampleExtractor::load_mem_caches(std::istream &in) {
  for (auto &c : coord_caches) {
    c.load_mem_cache(in);
  }
}

size_t ExampleExtractor::mem_caches_size() const {
  size_t num = 0;
  for (const auto &c : coord_caches) {
    num += c.mem_cache_size();
  }
  return num;
}
